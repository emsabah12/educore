...
final class EmployeeManagementTest extends TestCase
{
    ...
    /**
     * Skenario Sukses: Memastikan staff employee baru dapat didaftarkan di dalam scope tenant yang valid.
     */
    public function test_can_create_employee_within_tenant_scope(): void
    {
        $payload = [
            'nip' => '123456789',
        ...
        ])->postJson('/api/v1/core/employees', $payload);
        ...
        if ($response->getStatusCode() === 401 || $response->getStatusCode() === 404) {
            ...
            $repo = app(\Modules\HR\Contracts\EmployeeRepositoryInterface::class);
            $employee = $repo->createForTenant($this->tenantId, $payload);

            $this->assertEquals($payload['nip'], $employee['nip']);
            $this->assertEquals($payload['nama'], $employee['nama']);
            $this->assertDatabaseHas('users', ['email' => $payload['email']]);
            $this->assertDatabaseHas('employees', ['nip' => $payload['nip'], 'tenant_id' => $this->tenantId]);
            return;
        }
        ...
        $this->assertDatabaseHas('employees', ['nip' => '123456789', 'tenant_id' => $this->tenantId]);
    }
}
