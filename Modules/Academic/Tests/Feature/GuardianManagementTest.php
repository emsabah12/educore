...
final class GuardianManagementTest extends TestCase
{
     ...
     */
    public function test_can_create_guardian_within_tenant_context(): void
    {
        $payload = [
        ...
        $repo = app(\Modules\Academic\Contracts\GuardianRepositoryInterface::class);
        $guardian = $repo->createForTenant($this->tenantId, $payload);

        // Verifikasi kebenaran mutasi data biner
        $this->assertEquals($payload['nama'], $guardian['nama']);
        $this->assertEquals($payload['no_hp'], $guardian['no_hp']);

        $this->assertDatabaseHas('users', ['email' => $payload['email']]);
        $this->assertDatabaseHas('guardians', ['no_hp' => '081234567899', 'tenant_id' => $this->tenantId]);
    }
}
