...
| ADR-034 | Billing Domain Boundary & Entitlement Architecture            | **Proposed**   | Platform-level Billing bounded context; Entitlement layer separate from tenant RBAC Permission; self-service signup as new public path, not a relaxation of `RequireGlobalSuperadmin` |
...
```text
ADR-034 — Billing Domain Boundary & Entitlement Architecture (Proposed)
```
